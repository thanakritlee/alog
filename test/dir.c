...
/* GIVEN that the program has never been ran before,
   ...
   THEN the .alog directory should be created,
     ...
     AND the .alog/.artefacts directory should be created
     AND the .alog/.artefacts/.log_edit_msg file should be created.  */
static void normal_create_alog_dir() {
	...
	if (stat("/tmp/.alog/.artefacts", &stat_buf) == -1 || !S_ISDIR(stat_buf.st_mode)) {
		fprintf(stderr_test_fp, "[%s...%s] Expect .alog/.artefacts directory to have been created.\n", name, case_name);
		exit_code = -1;
	}
	else if (!(stat_buf.st_mode & S_IRUSR &&
		  stat_buf.st_mode & S_IWUSR &&
		  ...
		  !(stat_buf.st_mode & S_IWOTH) &&
		  !(stat_buf.st_mode & S_IXOTH))) {
		fprintf(stderr_test_fp, "[%s...%s] Expect .alog/.artefacts directory to have permission 0700, but got 0%o.\n", name, case_name, stat_buf.st_mode & 0777);
		exit_code = -1;
	}

	/* Assert that the file /.alog/.artefacts/.log_edit_msg has been created.  */
	if (stat("/tmp/.alog/.artefacts/.log_edit_msg", &stat_buf) == -1 || !S_ISREG(stat_buf.st_mode)) {
		fprintf(stderr_test_fp, "[%s...%s] Expect .alog/.artefacts/.log_edit_msg file to have been created.\n", name, case_name);
		exit_code = -1;
	}
	else if (!(stat_buf.st_mode & S_IRUSR &&
		  stat_buf.st_mode & S_IWUSR &&
		  ...
		  !(stat_buf.st_mode & S_IWOTH) &&
		  !(stat_buf.st_mode & S_IXOTH))) {
		fprintf(stderr_test_fp, "[%s...%s] Expect .alog/.artefacts/.log_edit_msg file to have permission 0600, but got 0%o.\n", name, case_name, stat_buf.st_mode & 0777);
		exit_code = -1;
	}
