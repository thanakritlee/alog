...
/* Create .alog, .alog/.artefacts, .alog/.tmp directories, and .alog/.artefacts/.log_edit_msg
   file if they don't already exist.  */
int create_alog_dir(char *alog_path) {
	char alog_tmp_path[PATH_MAX];
	char alog_artefacts_path[PATH_MAX];
	char alog_edit_msg_path[PATH_MAX];
	strcpy(alog_tmp_path, alog_path);
	strcat(alog_tmp_path, "/.tmp");
	strcpy(alog_artefacts_path, alog_path);
	strcat(alog_artefacts_path, "/.artefacts");
	strcpy(alog_edit_msg_path, alog_path);
	strcat(alog_edit_msg_path, "/.artefacts/.log_edit_msg");
	...
	if (stat(alog_artefacts_path, &stat_buf) == -1) {
		/* .alog/.artefacts directory doesn't exist.
		   Create the directory.  */
		if (mkdir(alog_artefacts_path, 0700) == -1) {
			fprintf(stderr, "[ERROR] Failed to create .alog/.artefacts directory: %s\n", strerror(errno));
		}
	}

	if (stat(alog_edit_msg_path, &stat_buf) == -1) {
		/* .alog/.artefacts/.log_edit_msg doesn't exist.
		   Create the file.  */
		int fd = open(alog_edit_msg_path, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
		if (fd == -1) {
			fprintf(stderr, "[ERROR] Failed to create .alog/.artefacts/.log_edit_msg file: %s\n", strerror(errno));
			return -1;
		}
		close(fd);
	}
